...
generator = torch.load('log_generator/AED.pth',map_location=torch.device(device))

    
print("Generator: ", generator, "\n\n")

if incor_discri == True:
    discriminator = torch.load('log_discriminator/AED_dis.pth',map_location=torch.device(device))
...
for tar_label in range(40):
    for rep in range(repeat_time):
        ...
        if use_GPU == False:
            ...
            AM_ins = AM_ins.squeeze().detach().cpu().numpy()
        np.save('visu/AED/' + 'AM_' + str(shape_names[tar_label]) + '_' + str(rep) + '.npy',AM_ins)
        write_pointcloud('visu/AED/' + 'AM_' + str(shape_names[tar_label]) + '_' + str(rep) + '.ply',AM_ins)
        
# =============================================================================
#         act_rec = np.array(act_rec)
#         plt.plot(act_rec)
#         plt.savefig('visu/AED/activation_'+ shape_names[tar_label] +'.png')
# =============================================================================
