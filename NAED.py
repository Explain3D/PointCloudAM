...
batch_size = 32
output_folder = "output_NAED/" # folder path to save the results
...
print("Weight of latent 1 distance loss: ", f1_weight)
print("Weight of latent 2 distance loss: ", f2_weight)
    ...
    for batch_id, data in enumerate(trainDataLoader, 0):
        ...
        if d_loss_for_check < 0:
            ...
            #Train VAE, 3 losses are used:
                ...
                #4. latent distance loss: Chamfer or L2(have a try) on latent distance f2
            ...
            #Loss 3
            f1_fake = net.encoder1(output.permute(0,2,1))
            f1_fake_to_cal_dis = f1_fake.permute(0,2,1)#.unsqueeze(-1).repeat(1,1,3)               #[batch_size,512]  #new:[32,128,1024]
            f1_real = net.encoder1(data_for_VAE.permute(0,2,1))
            f1_real_to_cal_dis = f1_real.permute(0,2,1)#.unsqueeze(-1).repeat(1,1,3)         #[batch_size,512]  #new:[32,128,1024]
            feat1_dis, _ = chamfer_distance(f1_fake_to_cal_dis, f1_real_to_cal_dis)
            
            
            #Loss 4
            f2_fake = net.encoder2(f1_fake).unsqueeze(-1).permute(0,2,1)#.unsqueeze(-1).repeat(1,1,3)               #[batch_size,512]  #new:[32,128]
            f2_real = net.encoder2(f1_real).unsqueeze(-1).permute(0,2,1)#.unsqueeze(-1).repeat(1,1,3)         #[batch_size,512]  #new:[32,128,1024]
            feat2_dis, _ = chamfer_distance(f2_fake, f2_real)
            
            
            g_loss = (chamfer_dis + f1_weight * feat1_dis + f2_weight * feat2_dis) - d_weight * d_fake.mean()
            g_loss.backward()
            VAE_optimizer.step()
            
            #Add noise to discriminator if discriminator loss is approximating -1, which denotes discriminator is super dominating, need to be panelized
            ...
            print("Feature 1 loss: ", feat1_dis)
            print("Feature 2 loss: ", feat2_dis)
...
for i in range(num_epochs) :
    ...
    print("\nSaving parameters...\n")
    torch.save(net, 'log_generator/NAED.pth')
    torch.save(discriminator, 'log_discriminator/NAED_dis.pth')
