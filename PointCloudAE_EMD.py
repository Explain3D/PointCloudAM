...
batch_size = 32
output_folder = "output_EMD/" # folder path to save the results
...
net = model.PointCloudAE_4l_de(point_size,latent_size)
print(net)
...
def train_epoch():
    ...
    for batch_id, data in enumerate(trainDataLoader, 0):
        ...
        loss = torch.mean(earth_mover_distance(data, output, transpose=False))
        print("Train number ", batch_id ," batches, loss: ", loss)
        loss.backward()
        optimizer.step()
...
def test_epoch(): # test with all test set
    with torch.no_grad():
        ...
        for j, data in enumerate(testDataLoader, 0):
            ...
            print("Testing ", j, "batches, loss: ", loss)
            epoch_loss += loss
...
for i in range(100) :
    ...
    if test_loss < best_loss:
        ...
        torch.save(net, 'log_generator/best_model_l4_EMD.pth')
